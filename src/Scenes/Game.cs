...
public class Game : Node2D
{
	...
	public void Initialise(GameInfo gameInfo, HashSet<PlayerInfo> playerInfoList)
	{
		...
		if (gameInfo.GameType == GameType.Replay)
			UI.DisableSurrender();
		UI.Initialise();
		UI.ChatMessage(Player.GetName() + " joined the game!");
    ...
    void CreatePlayers(HashSet<PlayerInfo> playerInfoList)
    {
	...
	void CreateTimers()
	{
		var timerEntity = ComponentFactory.Instance().CreateTimer(ContextManager.GameInfo.TimerType, 
																  ...
																  Player.GetID());
		GameSystem.Game.Player.SetTimer(timerEntity);
	...
	{
		GameSystem.Game.Player.ResourceEntity = ComponentFactory.Instance().CreateResource(Player.GetID());
		GameSystem.Game.Player.Resource = GameSystem.EntityManager.GetComponent<GResource>(Player.ResourceEntity);
	...
	public override void _Process(float delta)
	{
		...
		{
			GameSystem.Game.Player.ProcessTimer();
			Enemy.ProcessTimer();
		}
		...
		{
			var aiAction = ai.GetAction(EncodeGameState(), (User)GameSystem.Game.Player.GetEnemyID());
			var action = GameSystem.Input.ConvertAction(aiAction, (User)GameSystem.Game.Player.GetEnemyID());
